...
CASSANDRA_INTEGRATION_TEST_F(VectorNestedSimpleTest, VectorOfSets) {
  ...
  CassVector* vec = cass_vector_new(CASS_VALUE_TYPE_SET, 3);
...
CASSANDRA_INTEGRATION_TEST_F(VectorNestedSimpleTest, VectorOfVectors) {
  ...
  // Test with C API if cass_vector_append_vector exists
  const char* query = "INSERT INTO nested_vectors (id, vecs) VALUES (?, ?)";
  CassStatement* stmt = cass_statement_new(query, 2);
  cass_statement_bind_int32(stmt, 0, 2);
  
  // Create outer vector
  CassVector* outer_vec = cass_vector_new(CASS_VALUE_TYPE_CUSTOM, 3);
  
  // Create inner vectors
  for (int i = 0; i < 3; ++i) {
    CassVector* inner_vec = cass_vector_new(CASS_VALUE_TYPE_INT, 2);
    cass_vector_append_int32(inner_vec, i * 10);
    cass_vector_append_int32(inner_vec, i * 10 + 1);
    cass_vector_append_vector(outer_vec, inner_vec);
    cass_vector_free(inner_vec);
  }
  
  cass_statement_bind_vector(stmt, 1, outer_vec);
  cass_vector_free(outer_vec);
  ...
  // This might fail if the driver doesn't fully support it yet
  if (rc != CASS_OK) {
    // Log but don't fail - nested vectors might not be fully implemented
    std::cout << "Note: cass_vector_append_vector may not be fully implemented yet" << std::endl;
  }
  
  // At least verify the CQL insert worked
  Result result2 = session_.execute("SELECT COUNT(*) FROM nested_vectors");
  ASSERT_GE(result2.first_row().column_by_name<BigInteger>("count").value(), 1);
}